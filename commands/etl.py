...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        extract_parser.add_argument(
            "quiet", action="store_false", default=True, help="Remote system name"
        )
        extract_parser.add_argument(
    ...
    def handle_load(self, *args, **options):
        ...
        remote_system.load(options.get("loaders", []), quiet=options.get("quiet", True))

    def handle_extract(self, *args, **options):
        remote_system = RemoteSystem.get_by_name(options["system"])
        if not remote_system:
            self.die("Invalid remote system: %s" % options["system"])
        remote_system.extract(options.get("extractors", []), quiet=options.get("quiet", True))
