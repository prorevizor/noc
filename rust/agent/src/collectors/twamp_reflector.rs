...
impl ClientSession {
    ...
    async fn send_server_greeting(&mut self) -> Result<(), Box<dyn Error>> {
    ...
    async fn recv_setup_response(&mut self, t: Duration) -> Result<(), Box<dyn Error>> {
        ...
        match sr.mode {
            ...
            _ => {
                ...
                Err("unsupported mode".into())
            }
        }
    }
    async fn auth_unathenticated(&mut self) -> Result<(), Box<dyn Error>> {
        log::debug!("[{}] Starting unauthenticated session", self.id);
        Ok(())
    }

    async fn send_server_start(&mut self) -> Result<(), Box<dyn Error>> {
    ...
    async fn send_accept_session(&mut self) -> Result<(), Box<dyn Error>> {
    ...
    async fn send_start_ack(&mut self) -> Result<(), Box<dyn Error>> {
        log::debug!("[{}] Sending Start-Ack", self.id);
        let msg = StartAck { accept: 0 };
        self.connection.write_frame(&msg).await?;
        Ok(())
    }
    async fn recv_stop_sessions(&mut self) -> Result<(), Box<dyn Error>> {
    ...
    fn get_reflector_port(&self) -> Result<u16, Box<dyn Error>> {
        match self.reflector_port {
            Some(port) => Ok(port),
            None => Err("socket not created".into()),
        }
    }
    async fn start_reflector(&mut self) -> Result<(), &'static str> {
        let id = self.id.clone();
    ...
    async fn reflect(id: String, port_channel: oneshot::Sender<u16>) -> Result<(), Box<dyn Error>> {
        ...
        if let Err(e) = port_channel.send(socket.local_port()?) {
            ...
            return Err("cannot send reflector port".into());
        }
        //
        let mut seq = 0u32;
        loop {
