...
from noc.services.netflowcollector.netflowserver import NetflowServer
from noc.services.netflowcollector.datastream import NetflowDataStreamClient
from noc.services.netflowcollector.sourceconfig import SourceConfig
from noc.core.ioloop.timers import PeriodicCallback


class NetflowCollectorService(TornadoService):
    name = "netflowcollector"
    leader_group_name = "netflowcollector-%(dc)s-%(node)s"
    pooled = True
                ...
                orjson.dumps(
                    {
                        "ts": timestamp,
                        "object": cfg.id,
                        "data": {"source": "netflow", "collector": config.pool, "message": message},
                    }
                ),
                stream=cfg.stream,
    ...
    async def get_object_mappings(self):
        ...
        client = NetflowDataStreamClient("cfgnetflow", service=self)
