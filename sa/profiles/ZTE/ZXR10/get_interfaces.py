...
class Script(BaseScript):
    ...
    def split_data_in_table_to_columns(self, rows):
        ...
        vl = {"id": [], "name": [], "pvid_ports": [], "untag_ports": [], "tag_ports": []}
        vlan_list = []
        for line in rows:
            ...
            if match_head:
                ...
                tag_ports_len = (
                    len(match_head.group("tag_ports")) + 4
                )  # Add 4 to last column to compensate trail spaces
    ...
    def split_interface_groups(self, data):
        ...
        rx_interface_group = re.compile(
            r"^(?P<int_group>.+(?:group|\/))(?P<first_num>\d+)-(?P<last_num>\d+)$"
        )
        result = []
        for interface in data:
            ...
            if match:
                step = 0
                for interface_group in range(
                    int(match.group("first_num")) - 1, int(match.group("last_num"))
                ):
                    result.append(
                        match.group("int_group") + str(int(match.group("first_num")) + step)
                    )
                    step = step + 1
            else:
