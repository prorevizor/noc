...
class Script(GetMetricsScript):
    ...
    def get_interface_metrics(self, metrics):
        for metric in metrics:
            ...
            self.set_metric(
                id=("Interface | Status | Admin", [f"noc::port::{ifname}"]),
                value=1 if istatus not in ["Shut Down", "linkError"] else 0,
                type="gauge",
                scale=1,
            )
            self.set_metric(
                id=("Interface | Status | Oper", [f"noc::port::{ifname}"]),
                value=1 if istatus not in ["Shut Down", "Link Error"] else 0,
                type="gauge",
                scale=1,
            )
            self.set_metric(
                id=("Interface | Load | In", [f"noc::port::{ifname}"]),
    ...
    def get_channel_udp_metrics(self, metrics):
        for metric in metrics:
            ...
            self.set_metric(
                id=("Multicast | Channel | Group | Count", [f"noc::port::1/1.{metric.ifindex}"]),
                labels=([f"noc::multicast::group::1/1.{metric.ifindex}"]),
                value=udp_ports,
                type="gauge",
                scale=1,
    ...
    def get_group_metrics(self, metrics):
        ...
        for metric in metrics:
            ...
            self.set_metric(
                id=("Multicast | Group | Status", [f"noc::port::{channel}/{mname}"]),
                labels=(
    ...
    def get_group_bitrate_in_metrics(self, metrics):
        for metric in metrics:
            ...
            try:
                ...
                input = 0
            self.set_metric(
                id=("Multicast | Group | Bitrate | In", [f"noc::port::{channel}/{mname}"]),
                labels=(
                    [f"noc::multicast::name::{mname}", f"noc::multicast::channel::1/1.{channel}"]
                ),
                value=input,
    ...
    def get_group_bitrate_out_metrics(self, metrics):
        for metric in metrics:
            ...
            try:
                ...
                input = 0
            self.set_metric(
                id=("Multicast | Group | Bitrate | Out", [f"noc::port::{channel}/{mname}"]),
                labels=(
                    [f"noc::multicast::name::{mname}", f"noc::multicast::channel::1/1.{channel}"]
                ),
                value=input,
